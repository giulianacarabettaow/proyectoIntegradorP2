let listaProducts ={
  
    productos: [
        {
            id:1,
            nombre: 'Alfombra Blanca',    
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen: '/images/products/alfombra-blanca.jpg',
        },
        {
            id:2,
            nombre:'Alfombra Trenzada',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/alfombra-trenzada.jpeg',
        },
        {
            id:3,
            nombre:'AlfombraUno',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/alfombraUno.webp',
        },
        {
            id:4,
            nombre:'Cubre Maceta',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/cubreMaceta.png',
        },
        {
            id:5,
            nombre:'Cafetera Moulinex',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/img-cafetera-moulinex.jpg',
        },
        {
            id:6,
            nombre:'MacBook Pro',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/img-macbook-pro-2019.jpg',
        },
        {
            id:7,
            nombre:'Samsung Galaxy S10',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/img-samsung-galaxy-s10.jpg',
        },
        {
            id:8,
            nombre:'Tv Samsung Smart',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/img-tv-samsung-smart.jpg',
        },
        {
            id:9,
            nombre:'Sofa Blanco',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/sofaBlanco.jpg',
        },
        {
            id:10,
            nombre:'Libertad',
            descripcion:'No importaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
            precio:'$200.000', 
            imagen:'/images/products/default-image.png',
        }

       //falta comentarios
    ]
}

module.exports = listaProducts;